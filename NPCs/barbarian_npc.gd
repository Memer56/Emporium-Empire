...
@onready var mesh = $Barbarian/Rig/Skeleton3D
@onready var ray_cast = $RayCast3D
@onready var animation_player = $Barbarian/AnimationPlayer
...
func find_correct_queue_pos():
	...
	match index:
		-1:
			...
			go_to_checkout_and_unload_items()
		0:
			# go_to_checkout_and_unload_items() and find_correct_queue_pos() are repeatedly calling each other 
...
func _on_decision_timeout():
	...
	if attempted_to_grab_item == 2 and current_num_of_items_held == 0:
		attempted_to_grab_item = 0
		item_positions.clear()
		choose_next_target()
	elif attempted_to_grab_item == 2 and current_num_of_items_held > 0:
		go_to_checkout_and_unload_items()
